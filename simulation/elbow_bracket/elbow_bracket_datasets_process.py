# -*- coding: utf-8 -*-
"""Convert elbow_bracket_i CAE/ODB samples to machine-learning CSV files.
...
Run with the Abaqus/CAE Python kernel::
    ...
    abaqus cae noGUI=elbow_bracket_datasets_process.py
...
SAMPLE_PATTERN = re.compile(r'^elbow_bracket_(\d+)$', re.IGNORECASE)
...
def process_sample(sample_id, sample_dir, output_root, args):
    cae_path = os.path.join(sample_dir, 'elbow_bracket_structure.cae')
    odb_path = os.path.join(sample_dir, 'elbow_bracket_results.odb')
...
def build_argument_parser():
    parser = argparse.ArgumentParser(
        description='Export elbow_bracket_i Abaqus samples to ML CSV files.'
    )
    parser.add_argument(
        '--input-dir',
        default='elbow_bracket',
        help='Root containing elbow_bracket_i folders. Default: ./elbow_bracket',
    )
    parser.add_argument(
        '--output-dir',
        default=None,
        help='Output root. Default: sibling elbow_bracket_processed.',
    )
    parser.add_argument(
        '--instance',
        default=None,
...
def main():
    ...
    if args.output_dir is None:
        output_root = os.path.join(
            os.path.dirname(input_dir), 'elbow_bracket_processed'
        )
    else:
    ...
    if not samples:
        raise RuntimeError(
            'No directories matching elbow_bracket_<integer> were found in %s.'
            % input_dir
        )
    ...
    progress = tqdm(
        samples,
        total=len(samples),
        desc='Processing elbow bracket samples',
        unit='sample',
        dynamic_ncols=True,
    )
    for sample_id, sample_dir in progress:
        progress.set_postfix(sample='elbow_bracket_%d' % sample_id)
        try:
            ...
            log(
                '[ok] elbow_bracket_%d: instance=%s odb_step=%s cae_step=%s '
            ...
            log('[error] elbow_bracket_%d failed: %s' % (sample_id, exc))
            if args.fail_fast:
                raise
    ...
    if failures:
        for sample_id, message, trace in failures:
            log('elbow_bracket_%d: %s' % (sample_id, message))
            log(trace)
        return 1
    return 0
